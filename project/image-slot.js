...
/* Neutralize the benign "ResizeObserver loop completed with undelivered
   notifications" warning at the source: defer every ResizeObserver callback to
   the next animation frame so a layout write in one callback can't synchronously
   re-trigger the observation cycle. Also belt-and-suspenders message filters. */
(function () {
  try {
    var Native = window.ResizeObserver;
    if (Native && !Native.__cwWrapped) {
      var Wrapped = function (cb) {
        return new Native(function (entries, observer) {
          window.requestAnimationFrame(function () {
            try { cb(entries, observer); } catch (e) { /* ignore */ }
          });
        });
      };
      Wrapped.prototype = Native.prototype;
      Wrapped.__cwWrapped = true;
      window.ResizeObserver = Wrapped;
    }
  } catch (e) { /* ignore */ }
  function isRO(m) { return typeof m === 'string' && m.indexOf('ResizeObserver loop') !== -1; }
  window.addEventListener('error', function (e) {
    if (e && isRO(e.message)) { e.stopImmediatePropagation(); e.preventDefault(); return false; }
  }, true);
  var _err = console.error;
  console.error = function () { if (isRO(arguments[0])) return; return _err.apply(console, arguments); };
})();
/* BEGIN USAGE */
/**
...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // frame's clamp range. Defer _render out of the observation cycle via rAF
      // so a layout write here can't re-trigger the observer synchronously (which
      // surfaces the benign "ResizeObserver loop" console warning).
      this._ro = new ResizeObserver(() => {
        if (this._roScheduled) return;
        this._roScheduled = true;
        requestAnimationFrame(() => { this._roScheduled = false; this._render(); });
      });
